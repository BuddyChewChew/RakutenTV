...
EPG_WINDOW_HOURS    = 72   # total guide length we want to build
EPG_CHUNK_MINUTES   = 360  # window size Rakuten's API accepts per request
PER_PAGE            = 100  # Rakuten rejected per_page=250 as invalid; paginate instead
MAX_PAGES_PER_WINDOW = 10   # safety cap in case a short/empty page is never returned
...
def fetch_with_retry(url: str, timeout: int = 30) -> requests.Response:
    ...
    Non-503 4xx responses are not retried (a malformed/rejected request
    won't succeed by asking again) — the response body is logged and the
    error is raised immediately so the real reason is visible in the log.
    ...
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        try:
            ...
            if status is not None and 400 <= status < 500 and status != 503:
                ...
                print(f"  HTTP {status} body: {body_preview}")
                raise  # client error: won't succeed on retry, propagate now
            last_exc = exc
...
def get_epg_windows():
    """
    Return a list of (start, end) datetime pairs covering EPG_WINDOW_HOURS,
    split into EPG_CHUNK_MINUTES-sized chunks.

    Rakuten's live_channels endpoint appears to reject requests spanning
    more than ~360 minutes in a single call (confirmed 400 even when
    epg_duration_minutes correctly matched a 72h start/end range), so we
    request the full guide in successive 6-hour windows and merge them.
    """
    now = datetime.now(timezone.utc).replace(minute=0, second=0, microsecond=0)
    total_end = now + timedelta(hours=EPG_WINDOW_HOURS)

    windows = []
    cursor = now
    while cursor < total_end:
        chunk_end = min(cursor + timedelta(minutes=EPG_CHUNK_MINUTES), total_end)
...
def build_live_channels_url(epg_start: datetime, epg_end: datetime, page: int) -> str:
    """Build a gizmo.rakuten.tv/v3/live_channels request URL for one page of one window."""
    duration_minutes = int((epg_end - epg_start).total_seconds() // 60)
    params = (
        ...
        f"&epg_duration_minutes={duration_minutes}"
...
def main():
    ...
    # 2. Build Rakuten EPG API URLs — one per 6-hour chunk (see get_epg_windows)
    windows = get_epg_windows()

    channels_data   = []
    seen_channel_ids = set()
    programme_data  = []
    seen_programmes = set()  # (channel_id, starts_at) — dedupe overlap between chunk boundaries

    print(f"\nFetching EPG data from Rakuten API ({len(windows)} windows) ...")
    for idx, (epg_start, epg_end) in enumerate(windows, 1):
        print(f"  [window {idx}/{len(windows)}] {epg_start.isoformat()} -> {epg_end.isoformat()}")

        page = 1
        while page <= MAX_PAGES_PER_WINDOW:
            api_url = build_live_channels_url(epg_start, epg_end, page)
            resp = fetch_with_retry(api_url)
            data = resp.json()["data"]
            print(f"    page {page}: {len(data)} channels")
            ...
            for channel in data:
                ...
                for item in channel.get("live_programs", []):
                    ...
                    dedup_key = (ch_id, starts_at)
                    if dedup_key in seen_programmes:
                        continue
                    seen_programmes.add(dedup_key)
    ...
    print(f"\nRetrieved {len(channels_data)} channels, {len(programme_data)} programmes\n")
